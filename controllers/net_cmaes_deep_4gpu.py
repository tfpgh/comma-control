...
class GPUWorker(mp.Process):
    ...
    def rollout_batch(self, population_params):
        ...
        # 2. Parse Controller Weights (2-layer network: 14→20→16→1)
        params = torch.tensor(
        ...
        for step in range(CONTEXT_LENGTH, COST_END_IDX):
            ...
            # Extended Future Lookahead (exploit all 50 available points)
            max_future_idx = min(step + 41, 550)

            # Near future (0.5s): mean of next 5 steps
            future_near = targets[:, step + 1 : min(step + 6, 550)]
            if future_near.shape[1] > 0:
                future_near_mean = future_near.mean(dim=1, keepdim=True)
            else:
                future_near_mean = targets[:, idx_now : idx_now + 1]

            # Mid future (1.5s): mean of steps 6-15
            future_mid = targets[:, step + 6 : min(step + 16, 550)]
            if future_mid.shape[1] > 0:
                future_mid_mean = future_mid.mean(dim=1, keepdim=True)
            else:
                future_mid_mean = future_near_mean

            # Far future (3.5s): mean of steps 16-40
            future_far = targets[:, step + 16 : max_future_idx]
            if future_far.shape[1] > 0:
                future_far_mean = future_far.mean(dim=1, keepdim=True)
            else:
                future_far_mean = future_mid_mean

            # Future statistics (curviness and peak)
            all_future = targets[:, step + 1 : max_future_idx]
            if all_future.shape[1] > 0:
                future_var = all_future.std(dim=1, keepdim=True, unbiased=False)
                future_max_abs = all_future.abs().max(dim=1, keepdim=True)[0]
            else:
                future_var = torch.zeros_like(targets[:, idx_now : idx_now + 1])
                future_max_abs = targets[:, idx_now : idx_now + 1].abs()
...
def main():
    ...
    print("Starting 4-GPU CMA-ES Deep (2-layer, Extended Lookahead).")
    print(f"Population: {POPULATION_SIZE} | Segments: {NUM_SEGMENTS}")

    # Calculate parameter count for 14→20→16→1 architecture
    num_params = (
        (INPUT_SIZE * HIDDEN_1)
        + HIDDEN_1
        + (HIDDEN_1 * HIDDEN_2)
    ...
    # Initialize from random (fresh start with new architecture)
    # x0 = np.random.randn(num_params) * 0.1
    x0 = np.load("deep_best_params.npy")

    es = cma.CMAEvolutionStrategy(
        x0, 0.08, {"popsize": POPULATION_SIZE, "maxiter": MAX_GENERATIONS}
    )
    ...
    try:
        while not es.stop():
            ...
            if gen_best < best_ever:
                best_ever = gen_best
                np.save("deep_best_params.npy", es.result.xbest)
            ...
            if es.countiter % 50 == 0:
                np.save(f"deep_checkpoint_{es.countiter}.npy", es.result.xbest)
