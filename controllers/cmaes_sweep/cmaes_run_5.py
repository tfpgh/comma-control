...
INPUT_LAYER_SIZE = 18  # Changing requires adding or removing actual inputs
...
class Controller(BaseController):
    ...
    def update(
        ...
        # futures: get first 10 raw points
        fp = future_plan.lataccel if future_plan.lataccel else []
        if len(fp) < 10:
            fp = list(fp) + [target_lataccel] * (10 - len(fp))
        future_raw = np.array(fp[:10])
