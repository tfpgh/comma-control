...
class GPUWorker(mp.Process):
    ...
    def run(self):
        ...
        if "CUDAExecutionProvider" not in active_providers:
            print(f"[GPU {self.device_id}] WARNING: ORT fallback to {active_providers}. Performance will be terrible.")
        ...
        self.name_states = next((n for n in self.input_names if 'state' in n), self.input_names[0])
        self.name_tokens = next((n for n in self.input_names if 'token' in n), self.input_names[1])
        self.name_output = self.output_names[0]
        
        print(f"[GPU {self.device_id}] Bindings: {self.name_states}, {self.name_tokens} -> {self.name_output}")
    ...
    def rollout_batch(self, population_params):
        ...
        for step in range(CONTEXT_LENGTH, COST_END_IDX):
            ...
            torch.searchsorted(self.bins, ctx_lataccel.contiguous(), out=self.onnx_token_buffer)
